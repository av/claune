...
app = Celery('tasks', broker='sqla+sqlite:///' + os.path.join(UPLOAD_DIR, 'celerydb.sqlite'))
app.conf.update(
...
def process_audio_task(raw_path: str, file_id: str):
    ...
    cmd = ["ffmpeg", "-y", "-i", raw_path, "-t", "1.5", "-c:a", "libvorbis", "-af", "loudnorm=I=-16:TP=-1.5:LRA=11,afade=t=in:ss=0:d=0.01,afade=t=out:st=1.49:d=0.01", processed_path]
    ...
    if client:
        try:
            ...
            completion = client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an AI that tags meme sounds. Return JSON with 'tags' (array of strings) and 'is_safe' (boolean)."},
                    {"role": "user", "content": f"Transcription: '{transcription}'. Generate 3-5 tags. Set is_safe to false if NSFW."}
                ]
            )
            ...
            # Embeddings
            emb_res = client.embeddings.create(input=" ".join(tags) + " " + transcription, model="text-embedding-ada-002")
    ...
    c.execute("INSERT INTO sounds VALUES (?, ?, ?, ?, ?, ?, ?)", 
              (file_id, processed_filename, json.dumps(tags), duration, is_safe, json.dumps(embedding), s3_url))
    conn.commit()
    conn.close()
